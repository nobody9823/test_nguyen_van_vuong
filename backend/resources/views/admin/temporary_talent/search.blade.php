    ...
    @if(count($props) <= 0) <p>承認待ちタレントは存在しません。</p>
        ...
        <table class="table temporary_talent_list">
            ...
            <tr class="temporary_talent">
                <td>{{ $prop->name }}</td>
                <td>{{ $prop->email }}</td>
                <td><a href="/admin/temporary_talent/{{ $prop }}" class="btn btn-primary btn-sm">詳細</a></td>
                <td>
                    <form action="{{ route('admin.temporary_talent.accept', ['temporary_talent' => $prop]) }}"
                        method="post">
                        @csrf
                        @method('POST')
                ...
                <td>
                    <form action="{{ route('admin.temporary_talent.reject', ['temporary_talent' => $prop]) }}"
                        method="post">
                        @csrf
                        @method('DELETE')
        ...
        <div class="row justify-content-center mb-5">
            ...
            <a class="btn btn-primary ml-4" href="{{ route('admin.temporary_talent.index') }}" role="button">Topへ戻る</a>
        </div>
        @endif
</div>
@section('script')
