...
<div class="card-header d-flex align-items-center">
    ...
    <form action="{{ route('admin.talent.search') }}" class="form-inline pr-3" method="post">
        @csrf
    ...
    <div class="text-right">
        <a href="{{ route('admin.talent.create') }}" class="btn btn-outline-success">新規作成</a>
    </div>
</div>

<div class="card-body">
    @if(count($props) <= 0) <p>タレントのデータが存在しません。</p>
        @else
        <table class="table talent_list">
            ...
            @foreach($props as $talent)
            <tr class="talent">
                <td>{{ $talent->name }}</td>
                <td>{{ $talent->email }}</td>
                <td><a class="btn btn-primary btn-sm"
                        href="{{ route('admin.talent.edit',['talent' => $talent]) }}">編集</a>
                <td>
                    <form action="{{ route('admin.talent.destroy',['talent' => $talent]) }}" method="POST">
                        @method('DELETE')
                        @csrf
        ...
        <div class="row justify-content-center mb-5">
            ...
            <a class="btn btn-primary ml-4" href="{{ route('admin.talent.index') }}" role="button">Topへ戻る</a>
        </div>
        @endif
</div>
@section('script')
