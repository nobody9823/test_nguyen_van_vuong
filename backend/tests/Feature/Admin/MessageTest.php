...
class MessageTest extends TestCase
{
    ...
    {
        $response = $this->actingAs($this->admin, 'admin')->get(route('admin.message.index'));
        $response->assertOk();
    ...
    {
        $response = $this->actingAs($this->admin, 'admin')->get(route("admin.message.show", ['message' => UserPlanCheering::first()]));
        $response->assertOk();
        $response->assertSee('メッセージ一覧');
    ...
    public function test_store_and_fileDownload_and_sendEmail()
    {
        ...
        $response = $this->actingAs($this->admin, 'admin')->post(route("admin.message_content.store", ['user_plan_cheering' => UserPlanCheering::first()]), [
            'content' => 'これはテストです',
            'file_path' => $file,
        ]);
        ...
        $response = $this->actingAs($this->admin, 'admin')->get(route('admin.message_content.file_download', [
