...
class ActivityReportTest extends TestCase
{
    ...
    public function testIndexActionAtActivityReport()
    {
        ...
        ->get(route('talent.activity_report.index'));
    ...
    public function testCreateActionAtActivityReport()
    {
        $this->withoutExceptionHandling();
        $response = $this->actingAs($this->talent, 'talent')
        ->get(route('talent.activity_report.create',[
    ...
    {
        $response = $this->actingAs($this->talent, 'talent')
        ->from(route('talent.activity_report.search', [
            'project' => $this->projects[0],
            'activity_reports' => $this->projects[0]
                                        ->activityReports()
                                        ->paginate(10)
        ]))
        ->post(route('talent.activity_report.store', [
            'project' => $this->projects[0],
            'title' => 'added test title',
            'content' => 'added test content'
        ]));

        $response->assertRedirect(route('talent.activity_report.search', [
                'project' => $this->projects[0],
                'activity_reports' => $this->projects[0]
                                            ->activityReports()
                                            ->paginate(10)
    ...
    public function testShowActionAtActivityReport()
    {
        $this->withoutExceptionHandling();
        $response = $this->actingAs($this->talent, 'talent')
        ->get(route('talent.activity_report.show', [
            'project' => $this->projects[0],
    ...
    public function testEditActionAtActivityReport()
    {
        $this->withoutExceptionHandling();
        $response = $this->actingAs($this->talent, 'talent')
        ->get(route('talent.activity_report.edit', [
            'project' => $this->projects[0],
            'activity_report' => $this->activityReports[0]
        ]));
    ...
    public function testUpdateActionAtActivityReport()
    {
        ...
        ->patch(route('talent.activity_report.update', [
            'project' => $this->projects[0],
            'activity_report' => $this->activityReports[0],
    ...
    public function testDestroyActionAtActivityReport()
    {
        ...
        ->from(route('talent.activity_report.search', [
            ...
            'activity_reports' => $this->projects[0]
                                        ->activityReports()
                                        ->paginate(10)
        ]))->delete(route('talent.activity_report.destroy', [
            'project' => $this->projects[0],
            'activity_report' => $this->activityReports[0]
        ]));
    ...
    public function testSearchActionAtActivityReport()
    {
        ...
        $response = $this->actingAs($this->talent, 'talent')
        ->from(route('talent.activity_report.index'))
        ->get(route('talent.activity_report.search', ['word' => 'test']));
