...
class SupporterCommentTest extends TestCase
{
        ...
        $this->talents = Talent::factory()
            ...
            ->state(new Sequence(
                [
                    ...
                    'email' => 'talent_1@example.com',
                ],
                [
                    'company_id' => $this->company->id,
                    'name' => 'test_talent_second',
                    'email' => 'talent_2@example.com',
                ],
                [
                    'company_id' => $this->company->id,
                    'name' => 'test_talent_third',
                    'email' => 'talent_3@example.com',
                ],
            ))->create();
    ...
    public function testSearchActionByWords()
    {
        ...
        $response = $this->actingAs($this->talents[0], 'talent')
            ->get(route('talent.supporter_comment.search', ['word' => 'test']));
    ...
    public function testSearchActionByProject()
    {
        ...
        $response = $this->actingAs($this->talents[0], 'talent')
            ->get(route('talent.supporter_comment.search', ['project_id' => $this->projects[0]->id]));
