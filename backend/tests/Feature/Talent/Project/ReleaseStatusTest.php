...
namespace Tests\Feature\Talent\Project;
...
class ReleaseStatusTest extends TestCase
{
    ...
    public function testProjectApprovalRequestIsSuccessAsDefaultStatus()
    {
        ...
        $response = $this->actingAs($this->talent, 'talent')
            ->from(route('talent.project.show', ['project' => $this->projects[0]]))
            ->get(route('talent.project.approval_request', ['project' => $this->projects[0]]));

        $response->assertRedirect(route('talent.project.show', ['project' => $this->projects[0]]));
    ...
    public function testProjectApprovalRequestIsSuccessAsUnderSuspensionStatus()
    {
        $this->withoutExceptionHandling();
        $response = $this->actingAs($this->talent, 'talent')
            ->from(route('talent.project.show', ['project' => $this->projects[3]]))
            ->get(route('talent.project.approval_request', ['project' => $this->projects[3]]));

        $response->assertRedirect(route('talent.project.show', ['project' => $this->projects[3]]));
        $response->assertSessionHas('flash_message', '承認申請が完了しました。');
    }
    ...
    public function testProjectApprovalRequestIsSuccessAsSendBack()
    {
        $this->withoutExceptionHandling();
        $response = $this->actingAs($this->talent, 'talent')
            ->from(route('talent.project.show', ['project' => $this->projects[4]]))
            ->get(route('talent.project.approval_request', ['project' => $this->projects[4]]));

        $response->assertRedirect(route('talent.project.show', ['project' => $this->projects[4]]));
        $response->assertSessionHas('flash_message', '承認申請が完了しました。');
    }
}
