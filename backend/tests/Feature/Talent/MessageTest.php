...
class MessageTest extends TestCase
{
    ...
    {
        $response = $this->actingAs($this->talent, 'talent')->get(route('talent.message.index'));
        $response->assertOk();
    ...
    {
        $response = $this->actingAs($this->talent, 'talent')->get(route("talent.message.show", ['message' => UserPlanCheering::first()]));
        $response->assertOk();
        $response->assertSee('メッセージ一覧');
    ...
    public function test_store_and_fileDownload_and_sendEmail()
    {
        ...
        $response = $this->actingAs($this->talent, 'talent')->post(route("talent.message_content.store", ['user_plan_cheering' => UserPlanCheering::first()]), [
            'content' => 'これはテストです',
            'file_path' => $file,
        ]);
        ...
        $response = $this->actingAs($this->talent, 'talent')->get(route('talent.message_content.file_download', [
