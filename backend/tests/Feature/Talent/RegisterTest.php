...
class RegisterTest extends TestCase
{
    ...
    {
        $response = $this->get('talent/pre_create');
    ...
    {
        $response = $this->post('talent/pre_register', ['email' => $this->email]);

        $response->assertStatus(200)->assertViewIs('talent.pre_registered'); //validationに引っかからず正しく200が返ってきているか
    ...
    */
    ...
    {
        $response = $this->get(route('talent.create', ['token' => $token]));
    ...
    {
        $response = $this->post(route('talent.register',['token' => $this->email_verification->token]), $this->data);

        Mail::assertSent(RegisterFinished::class);
        $response->assertRedirect(route('talent.register_finished'));
        $this->assertDatabaseHas('temporary_talents',[
            'email' => 'test@test.com',
        ]);
    }
    ...
    {
        $response = $this->post(route('talent.register',['token' => 9999999999]), $this->data);

        Mail::assertNotSent(RegisterFinished::class);
        $response->assertRedirect(route('talent.pre_create'));
        $this->assertDatabaseMissing('temporary_talents', [
            'email' => 'test@test.com',
        ]);
    }
}
