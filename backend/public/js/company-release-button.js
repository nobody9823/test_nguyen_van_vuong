...
// ajaxの処理
function ToAjax(CompanyId, IsReleased){
    //完了を知らせるためにDeferredオブジェクトを生成しそれを返す
    var deferred = new $.Deferred();

    $.ajax({
        url: '/admin/company/' + CompanyId + '/release/' + IsReleased,
        type: 'GET',
        data: {'id': CompanyId, 'is_released': IsReleased},
        ...
        success: function(msg){
            if (msg === "success"){
                ...
                alert("公開には「社名」「メールアドレス」の記入が必須です。");
            } else {
                alert("公開に失敗しました。");
            }
        }
