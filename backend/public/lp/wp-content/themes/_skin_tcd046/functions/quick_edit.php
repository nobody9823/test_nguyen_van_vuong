...
function custom_quick_edit( $column_name, $post_type ) {
  // クイック編集項目と$column_nameと同じではないため1度だけ出力させる
  static $print_nonce = true;
  if ( $print_nonce ) {
    $print_nonce = false;

    // seo
    if ( in_array( $post_type, array( 'post', 'page', 'news', 'campaign', 'course', 'staff', 'voice' ) ) ) {
...
<?php
    ...
    // course
    if ( $post_type == 'course' ) {
      echo '<input type="hidden" name="course_front_meta_box_nonce" value="', wp_create_nonce( 'course_cf2.php' ), '" />';
?>
<fieldset class="inline-edit-col-right">
  <div class="inline-edit-col column-recommend">
    <div class="inline-edit-group">
      <label><input type="checkbox" name="banner_show_front_page" value="1" /> <?php _e( 'Display on the front page', 'tcd-w' );  ?></label>
    </div>
  </div>
</fieldset>
<?php
...
function custom_quick_edit_values( $actions, $post ) {
	...
	// seo
    if ( in_array( $post->post_type, array( 'post', 'page', 'news', 'campaign', 'course', 'staff', 'voice' ) ) ) {
  ...
  if ( $post->post_type == 'course' ) {
		$meta_keys = array_merge( $meta_keys,  array( 'banner_show_front_page' ) );
	}
